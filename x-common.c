...
int tcp_client_get(char* prog, int serverFd, char* lFile, char* rFile) {
    ...
    if (send(serverFd, message, INPUT_MAX, 0) == -1) {
        printf("%s: failed to send get file size response\n", prog);
        return 0;
    }
    memset(message, 0, INPUT_MAX);
...
int tcp_client_put(char* prog, int serverFd, char* lFile, char* rFile) {
    ...
    if (send(serverFd, &fSize, sizeof(fSize), 0) == -1) {
        printf("%s: failed to transmit the file size\n", prog);
        return 0;
    }
    if (recv(serverFd, message, INPUT_MAX, 0) == -1) {
        printf("%s: failed to receive put file size response\n", prog);
        return 0;
    }
    if (strcmp(message, "ready") != 0) {
        printf("%s: received unexpected put file size response\n", prog);
        return 0;
    }
    memset(message, 0, INPUT_MAX);
    ...
    while (fread(message, sizeof(char), INPUT_MAX, fPtr) > 0) {
        if (send(serverFd, message, INPUT_MAX, 0) == -1) {
            printf("%s: failed to put the whole file\n", prog);
            return 0;
        }
...
int tcp_server_get(char* prog, int clientFd, char* fName, int eCheck) {
    ...
    if (recv(clientFd, message, INPUT_MAX, 0) == -1) {
        printf("%s: failed to receive get file size response\n", prog);
        return 0;
    }
    if (strcmp(message, "ready") != 0) {
        printf("%s: received unexpected get file size response\n", prog);
        return 0;
    }
    memset(message, 0, INPUT_MAX);
...
int tcp_server_put(char* prog, int clientFd, char* fName, int eCheck) {
    ...
    sprintf(message, "%s", "ready");
    if (send(clientFd, message, INPUT_MAX, 0) == -1) {
        printf("%s: failed to send put file contents response\n", prog);
        return 0;
    }
    memset(message, 0, INPUT_MAX);
    ...
    fPtr = fopen(fName, "a");
    if (fPtr == NULL) {
        printf("%s: failed to create the file\n", prog);
        sprintf(message, "%s", "error");
        if (send(clientFd, message, INPUT_MAX, 0) == -1) {
            printf("%s: failed to send file creation error\n", prog);
        }
        return 0;
    }
    memset(message, 0, INPUT_MAX);
    ...
    printf("%s: successfully received the file\n", prog);
