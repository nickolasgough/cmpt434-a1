...
void get_file(int clientFd, int serverFd, struct addrinfo serverInfo) {
    ...
    if (sendto(serverFd, message, INPUT_MAX, 0, serverInfo.ai_addr, serverInfo.ai_addrlen) == -1) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (strcmp(cmd, "get") == 0) {
                ...
                if (recvfrom(serverFd, &fSize, sizeof(fSize), 0, (struct sockaddr*) &serverAddr, &serverLen) == -1) {
                    printf("mixed-proxy: failed to receive file size\n");
                    continue;
                }
                ...
                sprintf(message, "%s", "ready");
                if (sendto(serverFd, message, INPUT_MAX, 0, (struct sockaddr*) &serverAddr, serverLen) == -1) {
                    printf("mixed-proxy: failed to send file size response\n");
                    continue;
                }
                memset(message, 0, INPUT_MAX);

                printf("mixed-proxy: receiving the file...\n");
                cPos = 0;
                while (fSize > 0) {
                    rAmount = fSize > INPUT_MAX ? INPUT_MAX : fSize;
                    rSize = recvfrom(serverFd, &fDest[cPos], rAmount, 0, (struct sockaddr*) &serverAddr, &serverLen);
                    if (rSize == -1) {
                        printf("mixed-proxy: failed to receive the whole file\n");
                        break;
                    }
                ...
                printf("mixed-proxy: file successfully received\n");
